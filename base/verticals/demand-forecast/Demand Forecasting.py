...
# DBTITLE 1,Overview
# MAGIC %md
# MAGIC ![Databricks Demand Forecast](https://www.sas.com/content/dam/SAS/en_us/image/sas-com/icons/navmenu/analytics-icon.png/_jcr_content/renditions/original "Databricks Demand Forecast")
...
# MAGIC * menu is stored at "/mnt/databricks-wesley/demo-data/chick-fil-a/menu"
# MAGIC * transaction is stored at "/mnt/databricks-wesley/demo-data/chick-fil-a/transactions_final"
# MAGIC * sales is stored at "/mnt/databricks-wesley/demo-data/chick-fil-a/sales_final"
# MAGIC * location is stored at "/mnt/databricks-wesley/demo-data/chick-fil-a/geolocations"

# COMMAND ----------

menu = spark.read.parquet('/mnt/wesley/dataset/Fintech/demandforecast/menu')
menu.createOrReplaceTempView("menu")

transactions = spark.read.parquet('/mnt/wesley/dataset/Fintech/demandforecast/transactions_final')
transactions.createOrReplaceTempView("transactions")

locations = spark.read.parquet('/mnt/wesley/dataset/Fintech/demandforecast/geolocations')
locations.createOrReplaceTempView("locations")

sales = spark.read.parquet('/mnt/wesley/dataset/Fintech/demandforecast/sales_final')
...
# DBTITLE 1,Generate a Temporary Derived Tables using CTAS
# MAGIC %sql create temporary view sales2 as 
# MAGIC   select from_unixtime(timestamp,'YYYY-MM-dd HH') as `ts`, * from sales
...
model.save("/mnt/wesley/model/demandforecast/lr")
...
# MAGIC %md #### Load the streaming dataframe
...
# MAGIC %md #### Make predictions using the model trained using the historical data

# COMMAND ----------

streaming_predictions = model.transform(streaming_training)
