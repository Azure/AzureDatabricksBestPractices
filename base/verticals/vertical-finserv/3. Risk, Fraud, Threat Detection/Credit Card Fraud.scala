...
display(dbutils.fs.ls("/mnt/wesley/dataset/Fintech/CreditCard/"))

// COMMAND ----------

// DBTITLE 1,CSV File contains 77,617,325 rows
val filePath = "dbfs:/mnt/wesley/dataset/Fintech/CreditCard/credit_card_data.txt"
println("Number of Rows: " + sc.textFile(filePath).count())

// COMMAND ----------

// DBTITLE 1,CSV File is 20.9 GB
val fileSize = dbutils.fs.ls("dbfs:/mnt/wesley/dataset/Fintech/CreditCard/credit_card_data.txt").map(_.size).sum / (Math.pow(1024.0,3))
println(f"Size of file in GB: $fileSize%2.2f")

// COMMAND ----------

// DBTITLE 1,Register Table (auth_data) From CSV
val rawAuth = sqlContext.read.format("csv").option("delimiter", "|").option("inferSchema", "true").load("dbfs:/mnt/wesley/dataset/Fintech/CreditCard/credit_card_data.txt")
