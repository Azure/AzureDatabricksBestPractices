...
labels = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load("/melanoma/training/meta/labels/labels.csv")
...
def cacheFilesAndReturn(images, subdir):
  ...
  for image_id in images:
    shutil.copyfile("/dbfs/melanoma/training/data/%s.jpg" % image_id, str(file_dir)+"%s.jpg" % image_id)
