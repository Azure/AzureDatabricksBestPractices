...
# MAGIC 
# MAGIC ![Architecture](https://s3-us-west-2.amazonaws.com/databricks-trial-rlgarris/flow.png)
# MAGIC 
# MAGIC - Conservatively, fraud steals $80 billion a year across all lines of insurance. (Coalition Against Insurance Fraud estimate).
# MAGIC - Fraud comprises about 10 percent of property-casualty insurance losses and loss adjustment expenses each year; and
# MAGIC - Property-casualty fraud thus equals about $34 billion each year. (Insurance Information Institute, September 2017)
# MAGIC - Fraud accounts for 5-10 percent of claims costs for U.S. and Canadian insurers. Nearly one-third of insurers (32 percent) say fraud was as high as 20 percent of claims costs
# MAGIC 
# MAGIC In this example, we will be working with some P&C auto insurance data to demonstrate how we can create a predictive model that predicts if an insurance claim is fraudulent or not. This will be a Binary Classification task, and we will be creating a Decision Tree model.
# MAGIC 
...
insurance_claims.printSchema()

# COMMAND ----------

insurance_claims.createOrReplaceTempView("insurance_claims3")
...
df = table("insurance_claims3")

# COMMAND ----------

df = df.drop("policy bind date")
df = df.drop("incident date")
...
# MAGIC 
# MAGIC Purpose of doing this is to see if it makes sense to include all string columns as input to our Machine Learning Model.
...
display(df.groupBy("fraud reported").count())
...
colsToDelete = ["policy number", "policy bind date", "insured zip", "incident location", "incident date"]
...
stages = [StringIndexer(inputCol = categoricalCol, outputCol = categoricalCol + "Index",handleInvalid = "keep") for categoricalCol in filteredStringColList]
stages
...
# Rename label column
transformed_df = transformed_df.withColumnRenamed('fraud reportedIndex', 'label')

# Keep relevant columns (original columns, features, labels)
originalCols = df.columns
selectedcols = ["label", "fraud reported", "features"] + originalCols
...
# MAGIC %md ## Step 6: Zooming in on Prediction Data
# MAGIC 
...
# MAGIC %scala
# MAGIC 
# MAGIC cvPredictions.registerTempTable("cvPredictions")
...
targetDF = cvPredictions.select("prediction", "total claim amount", "insured hobbies").filter("prediction = 1")
