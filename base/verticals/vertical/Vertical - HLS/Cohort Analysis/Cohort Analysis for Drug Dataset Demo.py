...
# MAGIC 
# MAGIC 
# MAGIC * [**Cohort Analysis for drug Dataset**](http://www.gregreda.com/2015/08/23/cohort-analysis-with-python/) is the use of data analytics to build cohort analysis and is...   
# MAGIC   * Built on top of Databricks Platform
# MAGIC * This demo...  
# MAGIC   * demonstrates a cohort analysis between the incident filed for the cohort group as per the reported in incident date.  We use drug administration dataset from the [Open FDA site](https://open.fda.gov/drug/).
...
# DBTITLE 1,Step1: Ingest FDA Drug Data to Notebook
# MAGIC %md 
# MAGIC 
# MAGIC - We will download the drug dataset hosted at  [** FDA **](http://www.casact.org/research/index.cfm?fa=loss_reserves_data)
...
df = sc.wholeTextFiles('/mnt/azure/dataset/medicare/drugcohort/*.json').flatMap(lambda x: json.loads(x[1])).toDF()
df.registerTempTable("drug")
display(df)

# COMMAND ----------

# MAGIC %md ### Step2: Enrich the data to get additional insigts to drug dataset
# MAGIC - We count the number of data points and find the cohort group and cohort period.

# COMMAND ----------

rep_period = sqlContext.sql("select *, substr(report_date,0,6) as report_period from drug")
...
# MAGIC %md ###Step3: Explore FDA drug Data 
...
# MAGIC 
# MAGIC However, we can also see that the 2015-02 and 2015-03 cohort is the strongest, which enables us to ask targeted questions about this cohort compared to others -- what other attributes (besides first recall month) do these firms share which might be causing them to still recall high number of products? How were the majority of these products recalled by the firms? Was there a specific reason that caused the firm to recall such high number of products? The answers to these questions would inform future drug administration efforts.
# MAGIC 
# MAGIC Also it can be observed from the time the data is analyzed the number of cohort period decreases after each passing cohort group. We can infer that the administration is taking the right steps to reduces harmful drug recall incidence to the market. 