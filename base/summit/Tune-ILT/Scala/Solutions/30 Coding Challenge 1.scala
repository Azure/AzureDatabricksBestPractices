...
val trxSchema = "amount decimal(38,18), city_id integer"
val trxDF = spark.read.schema(trxSchema).parquet(tempTrxPath)

val citySchema = "city_id integer, country string, cityState string"
val cityDF = spark.read.schema(citySchema).parquet(tempCityPath)

val finalDF = trxDF.join(cityDF, "city_id")
