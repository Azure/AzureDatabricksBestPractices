...
val sourceFile = "/mnt/training/dataframes/tuning/people.csv"
val destFile = userhome + "/tmp/people.parquet"
...
val df1 = dedupedDF.select("firstName" , "ssn")
val df2 = dedupedDF.select("birthDate" , "ssn", "salary")

// COMMAND ----------

// ANSWER
val dfJoined = df1.join(df2, ("ssn"))

// COMMAND ----------

// ANSWER
dbutils.fs.rm(userhome + "tmp/people2.parquet", recurse=true)

dfJoined.write
  .mode(SaveMode.Overwrite)
  .option("compression", "snappy")
  .parquet(userhome + "tmp/people2.parquet")
...
// ANSWER
// there is a skewed key in `ssn` causing the slow join... this key is `555555555`

val df1Out = df1.filter("ssn != 555555555")
val df2Out = df2.filter("ssn != 555555555")
val df1In = df1.filter("ssn = 555555555")
val df2In = df2.filter("ssn = 555555555")

// COMMAND ----------

// ANSWER
val dfOutJoined = df1Out.join(df2Out, ("ssn"))
val dfInJoined = df1In.join(broadcast(df2In), ("ssn"))
val dfJoined2 = dfOutJoined.union(dfInJoined)
