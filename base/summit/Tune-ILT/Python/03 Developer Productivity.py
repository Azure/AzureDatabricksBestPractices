...
(df, total2014Slow, duration14Slow) = benchmarkCount(
  lambda : trx2014DF
)
...
(df, total2017Slow, duration17Slow) = benchmarkCount(
  lambda : trx2017DF
)
...
trx2014DF.FILL_IN    # write out a sample of the 2014 data to disk

trx2017DF.FILL_IN    # write out a sample of the 2017 data to disk
...
df = (spark
  .FILL_IN # Create the DataFrame
  .FILL_IN # Convert to a DataFrame and name each column
  .FILL_IN # Aggregate by the column "chocolate"
  .FILL_IN # Count each aggregate
)
chocolateCount = df.FILL_IN # Extract the total number of choclate candies
otherCount =     df.FILL_IN # Extract the total number of non-choclate candies
