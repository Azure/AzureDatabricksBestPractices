...
udfDF = initDF.select(
  upperUDF(initDF["firstName"]),
  lowerUDF(initDF["middleName"]), 
  upperUDF(initDF["lastName"]), 
  lowerUDF(initDF["gender"])
).distinct()

%timeit -n1 -r1 udfDF.count()
...
noUDF = initDF.select(
  upper(initDF["firstName"]),
  lower(initDF["middleName"]), 
  upper(initDF["lastName"]), 
  lower(initDF["gender"])
).distinct()

%timeit -n1 -r1 noUDF.count()
...
ipRangesDF = spark.read.json("/mnt/training/dataframes/country_ip_ranges.json")
transactionDF = spark.read.json("/mnt/training/dataframes/transaction_ips.json")
...
ipByCountry = transactionDF.join(
  ipRangesDF, 
...
ipByCountryBroadcasted = ipRangesDF.join(
