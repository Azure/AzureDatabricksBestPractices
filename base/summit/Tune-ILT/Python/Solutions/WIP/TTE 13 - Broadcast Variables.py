...
names = spark.read.parquet("/mnt/training/ssn/names.parquet")
...
test = names.join(people, ["firstName"])
test.show()
...
bcName = broadcast(names)

# COMMAND ----------

peopleNamesBcast = people2.join(bcName, names["firstName"] == people2["firstName"])
...
broadcastedSQLDF = spark.sql("SELECT /*+ BROADCASTJOIN(names) */* FROM names INNER JOIN people2 on  names.firstName=people2.firstName")
...
joinedDF2 = names.join(broadcast(people), ["firstName"])
