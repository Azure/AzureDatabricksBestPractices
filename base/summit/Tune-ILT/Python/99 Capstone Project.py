...
def validateDenormalize(year, df):
  validateYourAnswer("04.{}.A) Has 8 columns".format(year), 1276280174, len(df.columns))

  schemaStr = str(df.schema)
  validateYourAnswer("04.{}.B) Contains transacted_at [Timestamp]".format(year), 646192812, "transacted_at,TimestampType" in schemaStr)
  validateYourAnswer("04.{}.C) Contains description [String]".format(year), 646192812, "description,StringType" in schemaStr)
  validateYourAnswer("04.{}.D) Contains amount [Decimal]".format(year), 646192812, "amount,DecimalType" in schemaStr)
  validateYourAnswer("04.{}.E) Contains city [String]".format(year), 646192812, "city,StringType" in schemaStr)
  validateYourAnswer("04.{}.F) Contains state [State]".format(year), 646192812, "state,StringType" in schemaStr)
  validateYourAnswer("04.{}.G) Contains state_abv [String]".format(year), 646192812, "state_abv,StringType" in schemaStr)
  validateYourAnswer("04.{}.H) Contains country [String]".format(year), 646192812, "country,StringType" in schemaStr)
  validateYourAnswer("04.{}.I) Contains retailer [String]".format(year), 646192812, "retailer,StringType" in schemaStr)
...
tempDir = "{}/tuning/capstone".format(username)
...
(df, total, duration) = benchmarkCount(lambda: dfAll)

print("Duration:      {:,} seconds".format(int(duration/1000.0)))
print("Total Records: {}".format(total))

# COMMAND ----------

clearYourResults()
validateYourAnswer("06) Final Record Count", 2074954664, total)
validateYourAnswer("06.A) Less than 20 seconds", 646192812, duration < 20*1000)
validateYourAnswer("06.B) Less than 2 seconds", 646192812, duration < 2*1000)
summarizeYourResults()
