...
// MAGIC   if (runtimeVersions.size == 5) {
// MAGIC     val Array(dbrVersion, _, _, _, scalaVersion) = runtimeVersions
// MAGIC     (dbrVersion, scalaVersion.replace("scala", ""))
// MAGIC   } else if (runtimeVersions.size == 4) {
// MAGIC     val Array(dbrVersion, _, _, scalaVersion) = runtimeVersions
// MAGIC     (dbrVersion, scalaVersion.replace("scala", ""))
// MAGIC   } else if (runtimeVersions.size == 3) {
// MAGIC     val Array(dbrVersion, _, scalaVersion) = runtimeVersions
// MAGIC     (dbrVersion, scalaVersion.replace("scala", ""))
// MAGIC   } else if (runtimeVersions.size == 2) {
// MAGIC     val Array(dbrVersion, scalaVersion) = runtimeVersions
// MAGIC     (dbrVersion, scalaVersion.replace("scala", ""))
// MAGIC   } else {
// MAGIC     throw new IllegalArgumentException(s"""Dataset-Mounts is unable to parse the spark version from "${runtimeVersions.mkString(", ")}".""")
// MAGIC   }
// MAGIC }
