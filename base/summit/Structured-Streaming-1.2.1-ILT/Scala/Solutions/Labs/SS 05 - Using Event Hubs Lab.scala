...
val pcString = dbutils.widgets.get("CONNECTION_STRING")
val uniqueEHName = dbutils.widgets.get("EVENT_HUB_NAME")
...
lazy val eventHubsConf = Map(
  "eventhubs.connectionString" -> connectionString,         // Pass connectionString parameter to EventHubsConf object
  "eventhubs.startingposition" -> startingEventPosition,    // Define starting position from start of stream as a JSON string
  "setMaxEventsPerTrigger" -> "100"                         // Throttle Event Hubs' processing of the streams to, say, 100
)     

// COMMAND ----------

// TEST - Run this cell to test your solution.
lazy val csRegex = "Endpoint.*SharedAccessKeyName.*".r

dbTest("SS-05-partition",     true, csRegex.findFirstIn(connectionString) != None)
...
val initialDF = spark.readStream         // Get the DataStreamReader
...
dbTest("SS-05-partition",        true, initSchemaStr.contains("(partition,StringType,true)"))
dbTest("SS-05-offset",           true, initSchemaStr.contains("(offset,StringType,true)"))
dbTest("SS-05-sequenceNumber",   true, initSchemaStr.contains("(sequenceNumber,LongType,true)"))
dbTest("SS-05-enqueuedTime",     true, initSchemaStr.contains("(enqueuedTime,TimestampType,true)"))
dbTest("SS-05-publisher",        true, initSchemaStr.contains("(publisher,StringType,true)"))
dbTest("SS-05-partitionKey",     true, initSchemaStr.contains("(partitionKey,StringType,true)"))
dbTest("SS-05-properties",       true, initSchemaStr.contains("(properties,MapType(StringType,StringType,true),true)"))
dbTest("SS-05-systemProperties", true, initSchemaStr.contains("(systemProperties,MapType(StringType,StringType,true),true)"))
...
val cleanDF =  initialDF
  .withColumn("body", $"body".cast("STRING"))                                  // Cast "body" column to STRING
  .withColumn("ts_string", substring($"body", 2, 23))                          // Select the "value" column, pull substring(2,23) from it and rename to "ts_string"
  ...
  .withColumn("logData", regexp_extract($"body", """^.*\]\s+(.*)$""", 1))      // Select the "body" column and apply the regexp `"""^.*\]\s+(.*)$"""` and rename to  "logData"
...
dbTest("SS-05-schema-body",       true, schemaStr.contains("(body,StringType,true)"))
dbTest("SS-05-schema-ts_string",  true, schemaStr.contains("(ts_string,StringType,true)"))
dbTest("SS-05-schema-epoc",       true, schemaStr.contains("(epoc,LongType,true)"))
dbTest("SS-05-schema-capturedAt", true, schemaStr.contains("(capturedAt,TimestampType,true"))
dbTest("SS-05-schema-logData",    true, schemaStr.contains("(logData,StringType,true)"))
...
val ipDF = cleanDF
  ...
  .withColumn("ipClass"                                        // figure out class of IP address based on first two octets
     ...
     .otherwise("Invalid"))
  .groupBy( window($"capturedAt", "10 seconds").alias("time"), $"ipClass")    // gather in 10 second windows of "capturedAt", call them "time" and "ipClass"
  .count()                                                     // add up total
...
dbTest("SS-05-schema-ipClass", true, schemaStr.contains("(ipClass,StringType,false)"))
dbTest("SS-05-schema-count",   true, schemaStr.contains("(count,LongType,false)"))
dbTest("SS-05-schema-start",   true, schemaStr.contains("(start,TimestampType,true)"))
dbTest("SS-05-schema-end",     true, schemaStr.contains("(end,TimestampType,true)"))
...
// TEST - Run this cell to test your solution.
dbTest("SS-05-numActiveStreams", true, spark.streams.active.length > 0)
...
untilStreamIsReady("SS05-ipDF-s")
...
// TEST - Run this cell to test your solution.
dbTest("SS-05-numActiveStreams", 0, spark.streams.active.length)
