...
eventHubsConf  = {
  'eventhubs.connectionString' : connectionString,                   # Pass connectionString parameter to EventHubsConf object as a JSON string
  'eventhubs.startingPosition' : json.dumps(startingEventPosition),  # Define starting position from start of stream
...
cleanDF =  (initialDF
  .withColumn("body", col("body").cast("STRING"))                                  # Cast "body" column to STRING
  .withColumn("ts_string", substring(col("body"), 2, 23))                          # Select the "value" column, pull substring(2,23) from it and rename to "ts_string"
  ...
  .withColumn("logData", regexp_extract(col("body"), """^.*\]\s+(.*)$""", 1))      # Select the "body" column and apply the regexp `"""^.*\]\s+(.*)$"""` and rename to  "logData"
)
