...
# MAGIC 
# MAGIC 
# MAGIC As we work toward this, we will store the stages in a series of intermediate `list` objects. In this cell, we define the `list`, `stages`. We then add a `StringIndexer` and `OneHotEncoderEstimator` for each categorical feature.
...
# MAGIC 
# MAGIC 
# MAGIC In this cell, we define a new `list`, `stagesWithAssembler`. We then add a `VectorAssembler`.
...
# MAGIC 
# MAGIC 
# MAGIC First, we define a new (and final) `list`, `stagesComplete`. To this, we add our SparkML model, `LinearRegression`.
...
pipelinePath = userhome + "/linear_regression/pipeline_model"
