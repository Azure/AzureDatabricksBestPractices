...
class GridImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # This won't work, as we generate the images in parallel they are not numbered anymore. Need to fix the name
        # img = Image.open(os.path.join(self._data_path, '{}.png'.format(idx)))
        #print("pid is: " + pid)
        #print("x_center is: {} ".format(x_center))
        filename = "{}_{}_{}.png".format(pid.lower(),x_center,y_center)
        
        #img = Image.open(os.path.join(self._data_path, pid.lower() + '_'+int(x_center)+"_"+int(y_center)+'.png'))
        img = Image.open(os.path.join(self._data_path, filename))
