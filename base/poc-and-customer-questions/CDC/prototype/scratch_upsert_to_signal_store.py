...
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS paul.union_data_raw USING DELTA LOCATION '/mnt/databricks-paul/union-data-raw/'

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM paul.union_data_raw
...
def upsertToDelta(microBatchOutputDF, batch):
  ...
  spark.sql("""
    ...
    ON s.uid = t.uid
    WHEN MATCHED AND s.cdcTimestamp is not null THEN
       UPDATE SET 
           cdcTimestamp = s.cdcTimestamp,
           uid = s.uid,
           street = s.street,
           city = s.city,
           state = s.state,
     ...
     WHEN NOT MATCHED THEN INSERT 
           (clickTimestamp, exchangeID, publisher, creativeID, click, advertiserID, browser,
            geo, bidAmount, cdcTimestamp, uid, street, city, state, zipcode, firstname, lastname) 
            VALUES (s.clickTimestamp, s.exchangeID, s.publisher, s.creativeID, s.click, s.advertiserID, s.browser,
                   s.geo, s.bidAmount, s.cdcTimestamp, s.uid, s.street, s.city, s.state, s.zipcode, s.firstname, s.lastname) 
  """)
  ...
  spark.sql("""
    ...
    USING global_temp.stream_updates s
    ON s.uid = t.uid
     WHEN MATCHED AND s.clickTimestamp is not null THEN 
       UPDATE SET 
           clickTimestamp = s.clickTimestamp,
...
# MAGIC %sql 
# MAGIC SELECT count(*) FROM paul.signal_union WHERE street is not null AND bidAmount is not null

# COMMAND ----------

# MAGIC %sql 
# MAGIC SELECT * FROM paul.signal_union WHERE street is not null AND bidAmount is not null
