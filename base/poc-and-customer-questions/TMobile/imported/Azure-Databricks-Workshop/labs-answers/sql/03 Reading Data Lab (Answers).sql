...
-- DBTITLE 1,TO-DO
CREATE OR REPLACE TEMPORARY VIEW products_az
USING CSV
OPTIONS (path "/mnt/training-sources/initech/productsCsv/", 
         header "true",
         inferSchema "true"
...
-- DBTITLE 1,TO-DO
SELECT * FROM products_az where price > 1500
...
CREATE OR REPLACE TEMPORARY VIEW products (
  ...
  display string)
USING CSV
OPTIONS (path "/mnt/training-sources/initech/productsCsv/", header "true")

-- COMMAND ----------

-- MAGIC %sql select * from products
...
-- MAGIC USING parquet
-- MAGIC OPTIONS (path = "dbfs:/tmp/kyle-pistor/output/")
-- MAGIC AS 
