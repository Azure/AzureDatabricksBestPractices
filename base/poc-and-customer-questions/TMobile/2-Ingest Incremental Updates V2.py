...
#%sql
#SELECT * FROM v_dwh_association_cell_info_incremental

# COMMAND ----------

#%sql
#DESCRIBE TABLE v_dwh_association_cell_info_incremental
...
#%sql
#SELECT count(*), ct_SYS_CHANGE_OPERATION FROM v_dwh_association_cell_info_incremental
#GROUP BY ct_SYS_CHANGE_OPERATION 
...
df_all_updates = spark.sql("""
                      SELECT *
                      FROM v_dwh_association_cell_info_incremental
                      """).createOrReplaceTempView("all_changed_rows")
...
#%sql
'''
INSERT OVERWRITE TABLE association_oss_cell_info
SELECT
0 as oss_cell_info_id,
sr.site_cd, -- look up REF_DWH_CGI_SITE_SECTOR_LKP on OSS_CGI, NETWORK_HIERARCHY & OSS_VENDOR and populate (8 character)
sr.sector_cd, -- look up REF_DWH_CGI_SITE_SECTOR_LKP on OSS_CGI, NETWORK_HIERARCHY & OSS_VENDOR and populate (A0GPA, 11UAA, 21LAA)
case
when CAST(left(sr.sector_cd,1) as INT) is null then "GSM"
when CAST(left(sr.sector_cd,1) as INT) is not null and sr.sector_cd not like '%L%' THEN "UMTS"
when CAST(left(sr.sector_cd,1) as INT) IS NOT NULL and sr.sector_cd like '%L%' THEN "LTE"
else NULL
end as sector_technology_desc, -- decode using code below
v.oss_vendor -- key column
,v.network_hierarchy -- key column
        ,v.oss_cgi -- key column
     ,v.source_oss_server_desc
     ,v.oss_site_name
     ,v.oss_sector_name
     ,v.oss_cell_name
     ,oss_parent_equip_cd
     ,parent_equip_cd
     ,parent_equip_type_desc
     ,oss_mcc
     ,oss_mnc
     ,oss_lac
     ,oss_cell_id
     ,cell_operational_state_desc
     ,cell_administration_state_desc
     ,cell_barred_state_desc
     ,sector_status_desc
     ,sector_status_rank_preference
     ,oss_parent_obj_cd
     ,oss_parent_obj_type_desc
     ,oss_site_equip_cd
     ,oss_site_equip_type_desc
     ,oss_parent_equip_mkt_cd
     ,oss_mkt_name
     ,reason_desc
     ,alarm_text
     ,last_sync_dt
     ...
     ,oss_ecgi
     ,node_type_desc
 FROM V_DWH_ASSOCIATION_CELL_INFO v
 LEFT JOIN REF_DWH_CGI_SITE_SECTOR_LKP sr on (v.OSS_CGI = sr.OSS_CGI) and (v.NETWORK_HIERARCHY = sr.NETWORK_HIERARCHY) and (v.OSS_VENDOR = sr.OSS_VENDOR)
'''
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TEMPORARY VIEW association_oss_cell_info_updates AS
# MAGIC SELECT
...
# MAGIC  FROM all_changed_rows v
# MAGIC  LEFT JOIN REF_DWH_CGI_SITE_SECTOR_LKP sr on (v.OSS_CGI = sr.OSS_CGI) and (v.NETWORK_HIERARCHY = sr.NETWORK_HIERARCHY) and (v.OSS_VENDOR = sr.OSS_VENDOR)

# COMMAND ----------

display(spark.sql("SELECT * FROM association_oss_cell_info_changes"))

# COMMAND ----------

#%sql

#SELECT count(*) FROM association_oss_cell_info_changes
...
# MAGIC USING  association_oss_cell_info_updates inc
...
#%sql
'''
CREATE OR REPLACE TEMPORARY VIEW changed_association_oss_cell_info AS
SELECT a.* FROM association_oss_cell_info a
INNER JOIN updated_and_inserted_rows c on (c.OSS_CGI = a.OSS_CGI) and (c.NETWORK_HIERARCHY = a.NETWORK_HIERARCHY) and (c.OSS_VENDOR = a.OSS_VENDOR);
'''
